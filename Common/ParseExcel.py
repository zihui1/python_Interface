...
if __name__ == "__main__":
    ...
    # print(rows().value)
    # pe.getColumn(sheet,3)
    # for i in rows:
    #     print(i.value)
    # print(pe.getCellOfValue(sheet,rowNo=1,colsNo=1))
    # pe.writeCell(sheet,u'我爱祖国',rowNo=10,colsNo=10)
    # pe.writeCellCurrentTime(sheet,rowNo=10,colsNo=11)
    caseSheet = pe.getSheetByName(u'Sheet1')
    dataIsExecuteColumn = pe.getColumn(caseSheet, testCase_isExecute)
    print(dataIsExecuteColumn)
    # print("+++"+str(pe.getRowsNumber(sheet)))
    # for index in range(2, 3 + 1):
    #     # 获取数据驱动测试步骤表中第index行对象
    #     rowObj = pe.getRow(sheet, index)
    #     keyWord = rowObj[3 - 1].value
    #     print(keyWord)
